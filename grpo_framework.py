...
RESULT_NAME= 'grpo_train_df_result_test0.csv'
parent_output_dir = os.path.join(os.getcwd(), 'grpo_results')

def get_logps(trainer : GRPOTrainer):
...
def reward_hydrophobicity(completions, **kwargs):
    """
    Calculates a reward for protein sequences that is a balanced combination of
    hydrophobicity and a strict adherence to a target length.

    This version uses a Gaussian function to score the length, which scales the
    hydrophobicity score. This ensures that only sequences of the correct
    length receive a high reward, preventing the model from generating overly
    long sequences.

    Args:
        completions (list[str]): A list of generated protein sequences.
        **kwargs: Additional keyword arguments.

    Returns:
        list[float]: A list of standard Python floats, one for each sequence.
    """
    ...
    # Sigma for the Gaussian function. This controls how sharply the reward
    # drops off as the length deviates from the target. A smaller sigma
    # creates a narrower, stricter peak at the target length.
    length_sigma = 10

    for seq in completions:
        # Clean up the sequence by removing newlines and whitespace
        cleaned_seq = seq.replace('\n', '')
        seq_len = len(cleaned_seq)

        # Handle empty sequences to avoid division by zero
        if seq_len == 0:
            rewards.append(-20.0) # Assign a large negative reward
            continue

        # 1. Calculate the hydrophobicity score (the value to be maximized)
        hydro_score = (sum(hydrophobicity.get(aa, 0.0) for aa in cleaned_seq) / seq_len) +1

        # 2. Calculate the length score using a Gaussian (Normal) distribution.
        length_diff = seq_len - target_length
        length_score = np.exp(-0.5 * (length_diff / length_sigma)**2) + 1

        # 3. Calculate the final reward.
        final_reward = hydro_score * length_score

        # Explicitly cast the result to a standard Python float before appending.
        # This ensures the function returns a list of floats, not numpy.float64 objects.
        rewards.append(float(final_reward))

    # The print statement can still use numpy for convenient calculation of the mean
    # without affecting the function's return type.
    if completions:
        print("Cleaned Sequences:", completions)
        formatted_rewards = [f"{r:.3f}" for r in rewards]
        print(f"Rewards (average: {np.array(rewards).mean():.3f}):", formatted_rewards)

    return rewards


def get_rewards(completions: list[str], **kwargs) -> list[float]:
    """
    Calculates rewards for a list of sequences using a global lookup table.

    This function first cleans each sequence by removing newline characters.
    It then matches the cleaned sequence to a pre-computed global dictionary.
    If a match is found, the corresponding 'target_reg' value is returned.
    If not, it prints an error and assigns a reward of 0.

    Args:
        completions: A list of strings, where each string is a sequence.
        **kwargs: Catches any other keyword arguments from the trainer.

    Returns:
        A list of floats representing the reward for each sequence.
    """
    rewards = []
    for seq in completions:
        # Clean the sequence by removing any newline characters
        cleaned_seq = seq.replace('\n', '')

        # Look up the cleaned sequence in the dictionary
        reward = SEQUENCE_TO_REWARD_LOOKUP.get(cleaned_seq)

        if reward is not None:
            rewards.append(reward)
        else:
            # Print an error message if the cleaned sequence is not found
            print(f"ERROR: Sequence '{cleaned_seq}' not found in lookup table. Assigning reward of 0.0.", file=sys.stderr)
            rewards.append(0.0)

    return rewards


def parse_sequence_to_dict_grpo(df):
    """
    Parses sequences from a DataFrame into prompt-completion pairs.
    If a sequence does not start with 'M', the prompt is 'M' and the completion is the full sequence.
...
def main():
    df1 = pd.read_csv(filepath_or_buffer = 'gh114.csv')
    df2 = pd.read_csv(filepath_or_buffer = 'cm.csv')
    df3 = pd.read_csv(filepath_or_buffer = 'ppat.csv')
    dfs = [df1, df2, df3]

    for i, df in enumerate(dfs):
        if i == 1:
            break
        partitions = [df[df[f'part_{k}'] == 1].copy() for k in range(3)]
        # print(partitions[0].head())
        # print(partitions[1].head())
        # print(partitions[2].head())
        for j, partition in enumerate(partitions): 
            
            df_eval = partitions[j]
            df_train = pd.concat([partitions[k] for k in range(3) if k != j])

            # --- 1. Use the parsing function ---
            # Convert the train and eval DataFrames into dictionaries
            train_dict = parse_sequence_to_dict_grpo(df_train)
            eval_dict = parse_sequence_to_dict_grpo(df_eval)

            # --- 2. Convert dictionaries to datasets ---
            hf_train_dataset = Dataset.from_dict(train_dict)
            hf_eval_dataset = Dataset.from_dict(eval_dict)

            # --- 3. Verification (Optional) ---
            # This confirms the datasets were created correctly
            # print(f"\n--- Fold {j+1} for DF {i+1} ---")
            # print(f"Training samples: {len(hf_train_dataset)}")
            # print(f"Evaluation samples: {len(hf_eval_dataset)}")
            # print("Example training entry:", hf_train_dataset[0])
            ...
            model_output_dir = os.path.join(parent_output_dir, f"df_{i+1}_fold_{j+1}") # i is df number, j is eval fold

            # Config dict:
            training_args = GRPOConfig(output_dir=model_output_dir,
                           learning_rate = 1e-5,
    ...
    for i, df_train in enumerate(dfs):
        ...
        for _, row in hyp_file.iterrows():
            ...
            def compute_logps(trainer, valid_dict, hf_dataset):
                val_trainer = GRPOTrainer(model=trainer.model,
                                        args=trainer.args,
