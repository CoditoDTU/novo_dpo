...
def main_dpo_training_and_eval():
    ...
    PARENT_OUTPUT_DIR = os.path.join(os.getcwd(), 'DPO_logps', 'run_0')
    ...
    for j, df_train in enumerate(partitions):
        ...
        policy_logps = calculate_log_probs(model_output_dir, tokenizer, all_sequences, device)
    ...
    print(df_results.head())
    df_results.to_csv("dpo_results_with_logps.csv", index=False)
    print("\nFinal results saved to 'dpo_results_with_logps.csv'")
