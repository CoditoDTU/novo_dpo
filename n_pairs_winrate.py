    ...
    for n_samples in N_VALUES:
        for replicate in range(1, args.replicates + 1):
            ...
            hf_val_dataset = Dataset.from_dict(format_string_pairs_e(
                ...
                construct_pairs_e(df_val_sorted['target_reg'].to_list(), 0.0)
            ))
            n_valid_pairs = len(hf_val_dataset)

            base_model = AutoModelForCausalLM.from_pretrained("NorseDrunkenSailor/ProtGPT2-with-pad")
            logp_map = get_initial_logps_with_trainer(df_train_full['sequence'].to_list(), base_model, TOKENIZER)
            ...
            for modality in MODALITIES:
                ...
                rejected_df = pd.DataFrame({'rejected_sequences': hf_val_dataset['rejected'], 'rejected_logps': rejected_logps})
